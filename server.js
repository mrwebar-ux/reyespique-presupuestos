...
app.get('/api/siguiente', (req, res) => {
    try {
        ...
        const num = result.length > 0 ? parseInt(result[0].values[0][0]) : 3100;
        res.json({num: num});
    } catch(e) { res.status(500).json({error: e.message}); }
});
...
app.get('/api/clientes', (req, res) => {
    try {
        ...
        const clientes = result.length > 0 ? result[0].values.map(v => ({
            id: v[0], razon: v[1], rut: v[2], giro: v[3], planta: v[4], dir: v[5], contacto: v[6], email: v[7], tel: v[8]
        })) : [];
        res.json(clientes);
    } catch(e) { res.status(500).json({error: e.message}); }
});
...
app.get('/api/catalogo', (req, res) => {
    try {
        ...
        const cat = result.length > 0 ? result[0].values.map(v => ({
            id: v[0], desc: v[1], tipo: v[2], und: v[3], precio: v[4]
        })) : [];
        res.json(cat);
    } catch(e) { res.status(500).json({error: e.message}); }
});
...
app.post('/api/presupuestos', (req, res) => {
    try {
        ...
        db.run(`INSERT INTO presupuestos (id, num, desc, cliente_id, fecha, estado, tipo, validez, mo, mat, sub, gg, imp, util, net, iva, tot, created_at) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, 
        ...
        if (items && items.length > 0) {
            items.forEach((it, idx) => {
                ...
                [id, idx, it.desc, it.tipo, it.und, it.cant, it.tarifa]);
            });
        }
...
app.get('/api/presupuestos', (req, res) => {
    try {
        ...
        const pres = result.length > 0 ? result[0].values.map(v => ({id: v[0], num: v[1], desc: v[2], fecha: v[3], tot: v[4]})) : [];
        res.json(pres);
    } catch(e) { res.status(500).json({error: e.message}); }
});
