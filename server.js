...
app.post('/api/upload-drive', async (req, res) => {
    ...
    if (!driveService) return res.status(500).json({ success: false, error: "Credenciales de Google no configuradas." });
    try {
        const fileMetadata = { name: filename + '.doc', parents: ['1hdGCJUUcxc3h8iJrYVPqvRf7EktG5t3Z'] };
        const media = { mimeType: 'application/msword', body: Readable.from([content]) };
        const file = await driveService.files.create({ resource: fileMetadata, media: media, fields: 'id' });
...
initDB().then(() => app.listen(PORT, () => console.log("Servidor STRP SpA v16.0 Activo")));