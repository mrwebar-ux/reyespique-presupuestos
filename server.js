...
async function initDB() {
  ...
  app.get('/api/presupuestos', (req,res) => {
    ...
    const totalRow = q
      ? get("SELECT COUNT(*) as n FROM presupuestos p LEFT JOIN clientes c ON p.cliente_id=c.id WHERE p.num LIKE ? OR p.desc LIKE ? OR c.razon LIKE ?", [q,q,q])
      : get("SELECT COUNT(*) as n FROM presupuestos");
    res.json({ rows, total: totalRow?.n || 0, limit, offset });
  });
  ...
  app.get('/api/stats', (req,res) => {
    ...
    const neto  = get("SELECT SUM(net) as s FROM presupuestos WHERE strftime('%Y-%m',fecha)=strftime('%Y-%m','now')")?.s || 0;
    const sig   = get("SELECT valor FROM config WHERE clave='siguiente'")?.valor || '3100';
    const ultimos = all("SELECT p.*,c.razon,c.planta FROM presupuestos p LEFT JOIN clientes c ON p.cliente_id=c.id ORDER BY p.fecha DESC, CAST(p.num AS INTEGER) DESC LIMIT 5");
