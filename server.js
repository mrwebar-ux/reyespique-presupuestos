...
const DRIVE_SECRET_KEY = process.env.DRIVE_SECRET_KEY || "";
...
.company { font-size:13px; line-height:1.45; margin-top:8px; }
...
.totals { margin-left:auto; width:360px; margin-top:18px; border:1px solid #d1d5db; }
.row { display:flex; justify-content:space-between; padding:8px 12px; border-bottom:1px solid #e5e7eb; font-size:13px; }
.row.total { background:#111827; color:white; font-size:18px; font-weight:900; border-bottom:0; }
...
<div class="header">
  <div>
    <div class="logo">REYES & PIQUÉ</div>
    <div class="company">
      <strong>Servicio Técnico Reyes y Piqué SpA</strong><br>
      RUT: 76.602.175-1<br>
      Servicios técnicos, eléctricos, mantenimiento y automatización<br>
      Santiago, Chile
    </div>
  </div>
  <div class="box">
...
<div class="totals">
  <div class="row"><span>Mano de Obra</span><strong>${formatCLP(p.mo)}</strong></div>
  <div class="row"><span>Materiales</span><strong>${formatCLP(p.mat)}</strong></div>
  <div class="row"><span>Subcontratos</span><strong>${formatCLP(p.sub)}</strong></div>
  <div class="row"><span>Costo Directo</span><strong>${formatCLP(cd)}</strong></div>
  <div class="row"><span>Gastos Generales</span><strong>${formatCLP(p.gg)}</strong></div>
  <div class="row"><span>Imprevistos</span><strong>${formatCLP(p.imp)}</strong></div>
  <div class="row"><span>Utilidad</span><strong>${formatCLP(p.util)}</strong></div>
  <div class="row"><span>Subtotal Neto</span><strong>${formatCLP(p.net)}</strong></div>
  <div class="row"><span>IVA</span><strong>${formatCLP(p.iva)}</strong></div>
  <div class="row total"><span>TOTAL</span><strong>${formatCLP(p.tot)}</strong></div>
</div>
...
async function start() {
  ...
  app.get("/api/drive/status", async (req, res) => {
    try {
      if (!DRIVE_WEBHOOK_URL || !DRIVE_SECRET_KEY) {
        return res.json({ configured: false, ok: false, message: "Variables DRIVE_WEBHOOK_URL y DRIVE_SECRET_KEY no configuradas." });
      }

      const result = await sendToDrive({ action: "ping" });
