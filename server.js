...
:root { --blue-grad: linear-gradient(135deg, #0b1f4d 0%, #1f4e8c 55%, #2f6fb3 100%); --blue-dark:#0f172a; --blue-mid:#1e293b; --blue-soft:#eef2f6; }
body { font-family: Arial, Helvetica, sans-serif; color:var(--blue-dark); margin:0; padding:14mm; }
.header { display:flex; justify-content:space-between; border-bottom:4px solid var(--blue-mid); padding-bottom:16px; margin-bottom:24px; }
.logo { font-size:26px; font-weight:900; }
.company { font-size:12px; line-height:1.32; margin-top:0; }
.company-main strong { font-size:18px; color:var(--blue-dark); }
.company-main { line-height:1.32; }
.box { border:1px solid var(--blue-mid); padding:12px 16px; text-align:right; min-width:190px; }
h4 { background:#111827; color:#fff; padding:8px 10px; font-size:13px; text-transform:uppercase; margin:22px 0 10px; }
.grid { display:grid; grid-template-columns:1fr 1fr; gap:4px 14px; font-size:12px; }
...
.economic-wrap { margin-top:12px; width:100%; }
.economic-grid { display:grid; grid-template-columns:repeat(5, 1fr); gap:6px; width:100%; }
.economic-card { border:1px solid #e5e7eb; padding:7px 9px; background:#fff; min-height:42px; }
.economic-card .label { font-size:9px; text-transform:uppercase; color:#6b7280; margin-bottom:4px; font-weight:700; letter-spacing:.02em; }
.economic-card .value { font-size:14px; font-weight:800; color:var(--blue-dark); }
.economic-card.highlight-neto { background:#f8fafc; border:1.5px solid #64748b; }
.economic-card.highlight-iva { background:#eef2ff; border:1.5px solid #818cf8; }
.economic-card.highlight-total { background:#111827; border:1.5px solid #111827; }
...
async function start() {
  ...
  app.put("/api/presupuestos/:id", async (req, res) => {
    try {
      ...
      try { db.run("ROLLBACK"); } catch {}
      console.error(err);
      res.status(500).json({ error: "No fue posible actualizar el presupuesto." });
    }
  });
