...
async function initDB() {
    try {
        ...
        db.run(`INSERT OR IGNORE INTO config VALUES ('correlativo', '3100');`);
        ...
        db.run(`INSERT OR IGNORE INTO catalogo VALUES 
            ('k1', 'Cargo base por atención técnica', 'mo', 'HE', 143716),
...
// ENDPOINTS API
app.get('/api/siguiente', (req, res) => res.json({ correlativo: db.exec("SELECT valor FROM config WHERE clave='correlativo'")[0].values[0][0] }));
...
app.get('/api/presupuestos', (req, res) => {
    const r = db.exec("SELECT * FROM presupuestos ORDER BY num DESC");
    res.json(r.length ? r[0].values.map(v => ({ id: v[0], num: v[1], desc: v[2], cliente_id: v[3], tot: v[16] })) : []);
});
app.post('/api/config', (req, res) => { db.run("INSERT OR REPLACE INTO config VALUES (?,?)", [req.body.clave, req.body.valor]); saveDB(); res.json({success:true}); });
