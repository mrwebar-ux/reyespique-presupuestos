...
async function initDB(){
  ...
  let db=fs.existsSync(DB_PATH)?new SQL.Database(Buffer.from(fs.readFileSync(DB_PATH))):new SQL.Database();
  ...
  if(!get("SELECT 1 FROM config WHERE clave='nombre'")){
    [['nombre','Servicio Técnico Reyes y Piqué SpA'],['rut','76.602.175-1'],['giro','Contratista de molinos de trigo'],['dir','Obispo Vásquez Valencia 3104, Cerrillos, Santiago'],['email','mrwebar@gmail.com'],['tel','+56 9 94577868'],['elaboro','Daniela Gutiérrez'],['cargo','Encargada de Presupuestación'],['firma2','Manuel Reyes Webar — Ing. en Electricidad y Electrónica — Instalador Autorizado SEC Clase A'],['he','34000'],['hee','47905'],['base','143716'],['matPct','45'],['subPct','35'],['ivaPct','19'],['ggPct','12'],['impPct','5'],['utilPct','13'],['siguiente','3100']
    ].forEach(([k,v])=>db.run('INSERT OR IGNORE INTO config VALUES(?,?)',[k,v]));
    save();
  }
  if(!get("SELECT 1 FROM clientes WHERE id='c1'")){
    [['c1','Compañía Molinera San Cristóbal S.A.','','Molinería de trigo','Planta Santiago','Planta Santiago, Región Metropolitana','Sr. Williams Moya','',''],['c2','Compañía Molinera San Cristóbal S.A.','','Molinería de trigo','Planta Maipú','Planta Maipú, Región Metropolitana','Sr. Luis Gómez','',''],['c3','Compañía Molinera San Cristóbal S.A.','','Molinería de trigo','Planta Casablanca','Planta Casablanca','Sr. Cristian Mora del Prado','','']
    ].forEach(r=>db.run('INSERT INTO clientes VALUES(?,?,?,?,?,?,?,?,?)',r));
    save();
  }
  if(!get("SELECT 1 FROM catalogo WHERE id='k1'")){
    [['k1','Cargo base por atención','mo','HE',143716],['k2','Hora estándar técnico eléctrico (HE)','mo','HE',34000],['k3','Hora emergencia técnico (19:00-08:00)','mo','HE',47905],['k4','Servicio técnico electrónico con repuestos','mo','GL',295000],['k5','Traslados, fletes y colación','mo','GL',300000],['k6','Cable preensamblado Al 3x70+50mm','mat','m',4796],['k7','Cable libre halógeno 2.5mm H07Z1-K','mat','m',950],['k8','Cable control JZ-500 3G1.5mm','mat','m',2387],['k9','Interruptor MITSUBISHI NF125-CV 3x100A','mat','un.',58433],['k10','Contactor 50A bobina 220V','mat','un.',85000],['k11','Partidor suave AUCOM CSXI045 45kW','mat','un.',856422],['k12','Escalerilla alambre 400x65mm (tira 3m)','mat','tira',28500],['k13','Fungibles (amarras, cinta, tornillería)','mat','GL',50000],['k14','Materiales varios de montaje','mat','GL',300000],['k15','Ingeniero automatización — lazo control','sub','GL',280000],['k16','Materiales canalización tubería EMT 20mm','mat','GL',70000]
  ...
  app.post('/api/catalogo',(req,res)=>{const{desc,tipo,und,precio}=req.body;const id='k'+Date.now();run('INSERT INTO catalogo VALUES(?,?,?,?,?)',[id,desc||'',tipo||'mat',und||'GL',+precio||0]);res.json({id,desc,tipo,und,precio});});
  ...
  app.get('/api/presupuestos',(req,res)=>{
    ...
    const params=[];
    if(q){sql+=' WHERE (p.num LIKE ? OR p.desc LIKE ? OR c.razon LIKE ? OR c.planta LIKE ?)';params.push(q,q,q,q);}
    sql+=' ORDER BY p.fecha DESC,CAST(p.num AS INTEGER) DESC LIMIT ? OFFSET ?';
    params.push(limit,offset);
    const rows=all(sql,params);
    const tot=q?get('SELECT COUNT(*) as n FROM presupuestos p LEFT JOIN clientes c ON p.cliente_id=c.id WHERE (p.num LIKE ? OR p.desc LIKE ? OR c.razon LIKE ? OR c.planta LIKE ?)',[q,q,q,q]):get('SELECT COUNT(*) as n FROM presupuestos');
  ...
  app.post('/api/presupuestos',(req,res)=>{
    ...
    run("INSERT INTO presupuestos(id,num,desc,cliente_id,fecha,estado,tipo,validez,mo,mat,sub,gg,imp,util,net,iva,tot,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))",[id,num,desc||'',cliente_id||'',fecha||'',estado||'Emitido',tipo||'',validez||'7 días',+mo||0,+mat||0,+sub||0,+gg||0,+imp||0,+util||0,+net||0,+iva||0,+tot||0]);
    if(items&&items.length)items.forEach((it,i)=>{if(it)run('INSERT INTO items(pres_id,orden,desc,tipo,und,cant,tarifa) VALUES(?,?,?,?,?,?,?)',[id,i+1,String(it.desc||''),String(it.tipo||'mo'),String(it.und||'GL'),+it.cant||0,+it.tarifa||0]);});
  ...
  app.put('/api/presupuestos/:id',(req,res)=>{
    const{desc,cliente_id,fecha,estado,tipo,validez,mo,mat,sub,gg,imp,util,net,iva,tot,items}=req.body;
    run('UPDATE presupuestos SET desc=?,cliente_id=?,fecha=?,estado=?,tipo=?,validez=?,mo=?,mat=?,sub=?,gg=?,imp=?,util=?,net=?,iva=?,tot=? WHERE id=?',[desc||'',cliente_id||'',fecha||'',estado||'Emitido',tipo||'',validez||'7 días',+mo||0,+mat||0,+sub||0,+gg||0,+imp||0,+util||0,+net||0,+iva||0,+tot||0,req.params.id]);
    if(items){run('DELETE FROM items WHERE pres_id=?',[req.params.id]);items.forEach((it,i)=>{if(it)run('INSERT INTO items(pres_id,orden,desc,tipo,und,cant,tarifa) VALUES(?,?,?,?,?,?,?)',[req.params.id,i+1,String(it.desc||''),String(it.tipo||'mo'),String(it.und||'GL'),+it.cant||0,+it.tarifa||0]);});}
    res.json({ok:true});
  });
...
initDB().catch(err=>{console.error('Error crítico:',err.message);process.exit(1);});
